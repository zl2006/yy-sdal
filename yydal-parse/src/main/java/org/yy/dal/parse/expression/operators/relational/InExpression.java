...
public class InExpression implements Expression, SupportsOldOracleJoinSyntax {
    ...
    public void setOldOracleJoinSyntax(int oldOracleJoinSyntax) {
        ...
        if (oldOracleJoinSyntax < 0 || oldOracleJoinSyntax > 1) {
            throw new IllegalArgumentException("unexpected join type for oracle found with IN (type="
                + oldOracleJoinSyntax + ")");
        }
    }
    ...
    public String toString() {
        return (leftExpression == null ? leftItemsList : getLeftExpressionString()) + " " + ((not) ? "NOT " : "")
            + "IN " + rightItemsList + "";
    }
